...
Нужно доделать роутер и запуск в мейне, настроить докер, протестировать ручками что все работает, а вероятно оно не заработает с первого раза, покрыть тестами, но к сожалению я не успеваю к дате сдачи, поэтому это на таком этапе замораживается, но все равно выложу на гит
